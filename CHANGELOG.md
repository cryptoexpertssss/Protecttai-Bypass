...
- **Critical NSDL Jiffy Fix**: Resolved a crash/auto-close issue where static methods would cause a `NullPointerException` during logging.
- **Enhanced Primitive Support**: Added handling for `long`, `float`, `double`, etc., to prevent unboxing errors in various banking apps.
- **Universal Trace Logging**: Added more verbose logging for class discovery to help debug on devices without needing a full stack trace.
- **Target Expansion**: Added `p0.m` as a known target for NSDL Jiffy.
